...
def iterative_dict_learning_fista(shapes, n_components, dict_init=None, alpha=0.1,
    ...
    error = 0.5 * linalg.norm(np.matmul(learned_codes, dictionary) - shapes) ** 2 + alpha * linalg.norm(learned_codes, 1)
    error /= n_samples
...
if __name__ == "__main__":
    ...
    dim = 64  # dimension of shape data

    # to learn your dictionary and sparse coefficients for the training data
    train_data = np.random.random(size=(N, dim))
    learned_dict, learned_codes, losses, error = iterative_dict_learning_fista(train_data,
                                                                               n_components=32,
                                                                               alpha=0.1,
                                                                               batch_size=200,
                                                                               n_iter=100)

    print(learned_dict.shape, learned_codes.shape)

    # reconstructed data
    recon_shape = np.matmul(learned_codes, learned_dict)

    # to generate coefficients given a learned dictionary
    test_data = np.random.random(size=(100, dim))
    test_data_codes, _ = fast_ista(test_data, learned_dict, lmbda=0.1, max_iter=100)  # this lambda is the same as the alpha above

    print(test_data_codes.shape)
