...
void optMapMae(double *ia, double *ja, double *ib, double *jb, int n1, int n2, double *bestcostp)
{

    double *D1, *D, *C, big, y, bestcost ;
    int i,j, rot, yi, ct, stop, ii,jj,kk, N2, noTpnts, norot;
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    // input must be a arrays of double 
    if ( ( mxIsDouble(prhs[0]) != 1) | ( mxIsDouble(prhs[1]) != 1) | ( mxIsDouble(prhs[2]) != 1) | ( mxIsDouble(prhs[3]) != 1))
      mexErrMsgTxt("Input must be double.");

    
    // get the length of the input vectors 
    n1 = (mxGetM(prhs[0]) * mxGetN(prhs[0]));// + 1;
    n2 = (mxGetM(prhs[2]) * mxGetN(prhs[2])); // + 1;
    ...
    ib = mxGetPr(prhs[2]);
    jb = mxGetPr(prhs[3]); 
    bestcost= mxGetPr(plhs[0]);
    
    if ((ia == NULL)| (ja == NULL) | (ib == NULL) | (jb == NULL))
      mexErrMsgTxt("Could not convert input.");
    
    // call the C subroutine 
    if (n1>=n2)
       optMapMae(ia, ja, ib, jb, n1, n2, bestcost);
    else
       optMapMae(ib, jb, ia, ja, n2, n1, bestcost);
