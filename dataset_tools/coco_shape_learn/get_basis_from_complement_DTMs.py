...
out_dict = '{}/mask_fromDTM_complement_basis_m{}_n{}_a{:.2f}.npy'.format(save_data_root, mask_size, n_coeffs, alpha)
out_resampled_shape_file = '{}/train_mask_basis_complement_fromDTM_m{}.npy'.format(save_data_root, mask_size)
...
#     dist_bbox_in = dist_bbox_in / np.max(dist_bbox_in)  # range (0, 1)
#
#     complement_bbox = resized_dist_bbox - dist_bbox_in
#
#     dist_bbox = np.where(dist_bbox_in > 0, complement_bbox, -1.)  # in the range of -1, (0, 1)
#
...
#     # cv2.imshow('resized mask', dist_bbox.astype(np.uint8))
#     # cv2.imshow('complement dtm', dtm_show.astype(np.uint8))
