...
out_dict = '{}/single_fixed_fromMask_Dict_m{}_n{}_v{}_a{:.2f}.npy'.format(save_data_root, mask_size, n_coeffs, n_vertices, alpha)
out_resampled_shape_file = '{}/train_single_fixed_fromMask_m{}_v{}.npy'.format(save_data_root, mask_size, n_vertices)
...
for annotation in all_anns:
    ...
    dist_bbox = np.where(dist_bbox > 0, 255, 0)
    obj_contours, _ = cv2.findContours(dist_bbox.astype(np.uint8), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    ...
    canonic_contour = indexed_shape / np.array([mask_size, mask_size]) * np.array([gt_w, gt_h])

    shape_center = np.mean(canonic_contour, axis=0)
    norm_shape = (canonic_contour - shape_center) / np.array([gt_w / 2., gt_h / 2.])

    gt_rles = cocomask.frPyObjects([np.ndarray.flatten(canonic_contour).tolist()], gt_h, gt_w)
    gt_rle = cocomask.merge(gt_rles)  # ['counts'].decode('ascii')
    gt_m = cocomask.decode(gt_rle).astype(np.uint8) * 255  # in image domain

    # fig = plt.figure()
    # plt.title(cat_name)
    # plt.plot(canonic_contour[:, 0], -canonic_contour[:, 1], '-o', c='green', lw=2.5)
    # # plt.plot(recovered_shape[:, 0], -recovered_shape[:, 1], '--', c='red', lw=1.5)
    # plt.show()

    # For debug
    print(m_bbox.shape, gt_m.shape, gt_h, gt_w)
    show_img = np.concatenate([m_bbox, gt_m], axis=1)
    ...
    # print(contour.shape)
    # plt.title(cat_name)
    # plt.plot(original_polygons[:, 0], -original_polygons[:, 1], '-o', c='green', lw=2.5)
    # plt.plot(recovered_shape[:, 0], -recovered_shape[:, 1], '--', c='red', lw=1.5)
    # plt.show()
