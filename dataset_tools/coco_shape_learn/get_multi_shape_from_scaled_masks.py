...
out_dict = '{}/multi_fromMask_dict_wild_m{}_n{}_v{}_a{:.2f}.npy'.format(save_data_root, mask_size, n_coeffs, n_vertices, alpha)
out_resampled_shape_file = '{}/train_multi_wild_fromMask_m{}_v{}.npy'.format(save_data_root, mask_size, n_vertices)
...
for annotation in all_anns:
    ...
    print(contour.shape)
    rles = cocomask.frPyObjects([np.ndarray.flatten(uni_contour).tolist()], mask_size, mask_size)
    rle = cocomask.merge(rles)  # ['counts'].decode('ascii')
    m = cocomask.decode(rle).astype(np.uint8) * 255  # in image domain
    show_img = np.concatenate([dist_bbox, m], axis=1)
    cv2.imshow('original mask', show_img)
    cv2.waitKey()
