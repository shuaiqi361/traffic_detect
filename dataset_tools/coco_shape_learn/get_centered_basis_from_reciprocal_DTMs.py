...
out_dict = '{}/Centered_reciprocal_DTM_basis_m{}_n{}_a{:.2f}.npz'.format(save_data_root, mask_size, n_coeffs, alpha)
out_resampled_shape_file = '{}/train_reciprocal_DTM_basis_fromDTM_m{}.npy'.format(save_data_root, mask_size)
...
for annotation in all_anns:
    ...
    if annotation['area'] < 200:  # area 200 for 28 mask, 350 for 56 mask
        continue
    ...
    # dist_bbox_in = np.where(dist_bbox_in > 0, 1. / (dist_bbox_in + 0), 0.)
    # dist_bbox_show = np.clip(dist_bbox_in, 0, 1) * 255
    # # dist_bbox_in = dist_bbox_in / np.max(dist_bbox_in)
    # # dist_bbox_show = np.where(dist_bbox_in > 0, (1 - dist_bbox_in) * 255, 0)
    # cv2.imshow('complement dtm', dist_bbox_show.astype(np.uint8))
    #
    # if cv2.waitKey() & 0xFF == ord('q'):
    #     exit()

    dist_bbox = np.where(dist_bbox_in > 1e-5, np.clip(1. / (dist_bbox_in + 1e-5), 1e-5, 1), -1.)  # in the range of -1, (0, 1]

    dtm_show = np.where(dist_bbox > 0, dist_bbox, 0) * 255
