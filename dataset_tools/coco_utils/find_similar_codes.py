...
for annotation in all_anns:
    ...
    # add small purterbation to object shapes
    if counter_obj == 369:  # one example for demonstration the sensitivity of codes

        for k in range(6):
            noise_vec = np.random.rand(num_vertices, 2) * 15
            if k > 0:
                fixed_contour_p = fixed_contour + noise_vec  # apply the noise
            else:
                fixed_contour_p = fixed_contour * 1.0
            ...
            plt.title('Distribution of {} learned coefficients'.format(k))
            plt.show()
