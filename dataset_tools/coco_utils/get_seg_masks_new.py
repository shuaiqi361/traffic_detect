...
n_vertices = 64  # predefined number of polygonal vertices
n_coeffs = 64
alpha = 0.7
...
out_dict = '{}/train_dict_v{}_n{}_a{:.2f}.npy'.format(save_data_root, n_vertices, n_coeffs, alpha)
...
for annotation in all_anns:
    ...
    if len(polygons) < n_vertices * 0.3 * 2:
        counter_poor += 1
        continue
    else:
    ...
    shape_std = np.sqrt(np.sum(np.std(canonic_contour, axis=0) ** 2))
    norm_shape = (canonic_contour - shape_center) / shape_std
