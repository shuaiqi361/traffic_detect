...
for annotation in all_anns:
    ...
    # fixed_contour[fixed_contour[:, 0] >= w_img, 0] = w_img - 1
    # fixed_contour[fixed_contour[:, 0] < 0, 0] = 0
    # fixed_contour[fixed_contour[:, 1] >= h_img, 0] = h_img - 1
    # fixed_contour[fixed_contour[:, 1] < 0, 0] = 0
    # Using ground truth bboxes to bound the contours
    # fixed_contour[fixed_contour[:, 0] > gt_x1 + gt_w, 0] = gt_x1 + gt_w
    # fixed_contour[fixed_contour[:, 0] < gt_x1, 0] = gt_x1
    # fixed_contour[fixed_contour[:, 1] > gt_y1 + gt_h, 0] = gt_y1 + gt_h
    # fixed_contour[fixed_contour[:, 1] < gt_y1, 0] = gt_y1
    ...
    recon_contour = recon_contour * np.array([bbox_width / 2., bbox_height / 2.]) + bbox_center
